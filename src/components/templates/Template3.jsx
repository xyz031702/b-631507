...
const Template3 = ({ data }) => {
  ...
  return (
    <BaseTemplate data={data}>
        ...
        <div className="flex justify-between items-start mb-8">
          <div>
            <div className="bg-white text-blue-500 p-2 inline-block rounded">
              <h1 className="text-2xl font-bold">{yourCompany?.name || 'Your Company Name'}</h1>
            </div>
            <p className="mt-2">{yourCompany?.address || 'Your Company Address'}</p>
      ...
      <div className="p-8 rounded-lg border border-gray-300 -mt-18">
          ...
          <tbody>
            {(items || []).map((item, index) => (
              <tr key={index} className="border-t border-b">
                ...
                <td className="p-2 text-right">₹{((item.quantity || 0) * (item.amount || 0)).toFixed(2)}</td>
              </tr>
            ))}
          </tbody>
        </table>
