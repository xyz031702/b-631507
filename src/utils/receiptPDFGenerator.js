...
export const generateReceiptPDF = async (receiptData, templateNumber, dimensions = { width: '80mm', height: 'auto' }) => {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      const Template10 = (await import('../components/templates/Template10')).default;
      const ReactDOMServer = (await import('react-dom/server')).default;
      const React = (await import('react')).default;
      
      const receiptElement = React.createElement(Template10, { data: receiptData, isPrint: true });
